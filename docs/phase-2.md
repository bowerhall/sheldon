...
**Depends on: Phase 1**
**Goal: Kora takes autonomous action**
...
- Context bridging: Kora generates per-task `CLAUDE.md` from koramem (preferences, deploy target, relevant facts)
- Sandbox: isolated workspace, stripped environment (no Kora credentials), restricted tool set
...
- Integration with Phase 6: deployable artifacts (Dockerfile + k8s manifests) chain into self-extension pipeline

### 2. Apartment Hunter Skill (Days 3-5)
- SKILL.md: query koramem D9 + D8 + D11 for criteria
- WebSearch + WebFetch for listings
- Filter against budget, location, size from memory
- Generate Bewerbung using personal facts from koramem
...
- Pulls context from all relevant domains via koramem.Recall
...
- Reads D8 from koramem

### 5. Output Sanitizer (Day 7)
- Integrated into Claude Code Bridge (see [claude-code-bridge.md](claude-code-bridge.md#output-handling))
