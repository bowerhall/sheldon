...
Presigned S3 URLs generated by Sheldon returned `SignatureDoesNotMatch` errors when users clicked them. URLs worked fine from server, failed from browser.

### Debugging Timeline

| Step | Hypothesis | Finding |
|------|------------|---------|
| 1 | Wrong MinIO credentials | Credentials were correct, `mc` commands worked |
| 2 | Public endpoint not configured | Logs showed `publicEndpoint=s3.bowerhall.dev publicSSL=true` ✓ |
| 3 | DNS/connectivity issue | Container could reach `s3.bowerhall.dev` via wget ✓ |
| 4 | Cloudflare interfering | Presigned URLs worked via curl from server AND local machine ✓ |
| 5 | URL being modified somewhere | **Bingo** - ctrl+click worked, copy/paste failed |

### Root Cause

**Telegram decodes URL-encoded characters when displaying messages.**

```
Original URL:  ...Credential=sheldon%2F20260304%2Fus-east-1...
Displayed:     ...Credential=sheldon/20260304/us-east-1...
Copied:        ...Credential=sheldon/20260304/us-east-1...  ← WRONG
```

The S3 signature is calculated with `%2F` (encoded `/`). When Telegram displays the URL, it decodes `%2F` to `/`. Copying the displayed text gives you the decoded version, which has a different signature.

**Why ctrl+click worked**: The underlying hyperlink preserves the original encoded URL.

### Solution

Wrap presigned URLs in markdown code blocks:

```go
return fmt.Sprintf("PRESIGNED URL:\n\n```\n%s\n```\n\nCopy exactly as shown.", url)
```

Code blocks (`<pre>` in HTML) preserve exact characters, so copying works correctly.

### Key Debugging Insight

When "URL works on server but not in browser":
1. Test with `curl` from multiple locations (server, local, external)
2. If curl works everywhere, the URL itself is valid
3. Check how the URL is being transmitted/displayed to the user
4. Compare ctrl+click vs copy/paste behavior

### Related: Telegram Markdown-to-HTML Edge Cases

Other URL issues we fixed in the same codebase:

| Issue | Cause | Fix |
|-------|-------|-----|
| Underscores converted to italics | `file_name.svg` → `file<i>name.svg` | Extract URLs to placeholders before formatting |
| Bold markers in URLs | `**https://...` captured `**` | Exclude `*` from URL regex pattern |
